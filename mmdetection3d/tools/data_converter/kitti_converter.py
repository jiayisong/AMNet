...
def export_2d_annotation(root_path, info_path, mono3d=True):
    ...
    for info in mmcv.track_iter_progress(kitti_infos):
        coco_infos = get_2d_boxes(info, occluded=[-1, 0, 1, 2, 3], mono3d=mono3d)
        (height, width,
...
def get_2d_boxes(info, occluded, mono3d=True):
    ...
    for ann_idx, ann_rec in enumerate(ann_recs):
        ...
        if ann_rec['occluded'] == -1:
            ...
            box_corners = corners_3d[0]
            z = box_corners[:, 2]
            z[z < 0.1] = 0.1
            box_corners[:, 2] = z
            box_corners_in_image = points_cam2img(box_corners, P2)
            # box_corners_in_image: [8, 2]
            minxy = np.min(box_corners_in_image, axis=0, keepdims=False)
            maxxy = np.max(box_corners_in_image, axis=0, keepdims=False)

            min_x, min_y = minxy
            max_x, max_y = maxxy
            min_x = np.clip(min_x, 0, image_shape[1] - 1)
            max_x = np.clip(max_x, 0, image_shape[1] - 1)
            min_y = np.clip(min_y, 0, image_shape[0] - 1)
            max_y = np.clip(max_y, 0, image_shape[0] - 1)
        ...
        if mono3d and (repro_rec is not None) and (ann_rec['occluded'] != -1):
            ...
            if center3d[0, 2] < 0.1:
                print(center3d)
            center2d = points_cam2img(center3d, P2, with_depth=True)
...
def generate_record(ann_rec, x1, y1, x2, y2, sample_data_token, filename):
    ...
    if repro_rec['category_name'] not in kitti_categories:
        if repro_rec['category_name'] != 'DontCare':
            return None
        else:
            category_id = -1
            iscrowd = 1
    else:
        category_id = kitti_categories.index(repro_rec['category_name'])
        iscrowd = 0
    coco_rec['category_name'] = repro_rec['category_name']
    coco_rec['truncated'] = repro_rec['truncated']
    coco_rec['occluded'] = repro_rec['occluded']
    coco_rec['difficulty'] = repro_rec['difficulty']
    coco_rec['category_id'] = category_id
    coco_rec['bbox'] = [x1, y1, x2 - x1, y2 - y1]
    coco_rec['iscrowd'] = iscrowd
