...
def image_box_overlap(boxes, query_boxes, criterion):
    N = boxes.shape[0]
...
def bev_box_overlap(boxes, qboxes, gpu_ids, criterion=-1):
    from .rotate_iou import rotate_iou_gpu_eval
    riou = rotate_iou_gpu_eval(boxes, qboxes, criterion, gpu_ids)
    return riou
...
def d3_box_overlap(boxes, qboxes, gpu_ids, criterion=-1):
    from .rotate_iou import rotate_iou_gpu_eval
    rinc = rotate_iou_gpu_eval(boxes[:, [0, 2, 3, 5, 6]],
                               qboxes[:, [0, 2, 3, 5, 6]], 2, gpu_ids)
...
def calculate_iou_partly(gt_annos, dt_annos, metric, gpu_ids, num_parts=50):
    ...
    for num_part in split_parts:
        ...
        if metric == 0:
            ...
            overlap_part = image_box_overlap(gt_boxes, dt_boxes, -1)
        elif metric == 1:
            loc = np.concatenate(
            ...
            overlap_part = bev_box_overlap(gt_boxes,
                                           dt_boxes, gpu_ids).astype(np.float64)
        elif metric == 2:
...
def eval_class(gt_annos,
    ...
    rets = calculate_iou_partly(dt_annos, gt_annos, metric, gpu_ids, num_parts)
...
def do_coco_style_eval(gt_annos, dt_annos, current_classes, overlap_ranges, gpu_ids,
                       compute_aos):
    ...
    _, _ = do_eval(gt_annos, dt_annos,
                   current_classes, min_overlaps, gpu_ids,
                   compute_aos)
...
def kitti_eval(gt_annos,
    ...
    if 'bev' not in eval_types:
        table_data = [['Difficult', 'IOU', 'Class', 'R11_AP_2D', 'R40_AP_2D']]
    else:
        table_data = [
            ['Difficult', 'IOU', 'Class', 'R11_AP_2D', 'R11_AP_BEV', 'R11_AP_3D', 'R11_AOS', 'R40_AP_2D', 'R40_AP_BEV',
             'R40_AP_3D', 'R40_AOS']]
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            for k in range(len(difficulty)):
                if 'bev' not in eval_types:
                    table_data.append(
                        [difficulty[k], min_overlaps[i, 0, j], curcls_name, f'{mAP11_bbox[j, k, i]:.2f}',
                         f'{mAP40_bbox[j, k, i]:.2f}'])
                else:
                    table_data.append(
                        [difficulty[k], min_overlaps[i, 0, j], curcls_name, f'{mAP11_bbox[j, k, i]:.2f}',
                         f'{mAP11_bev[j, k, i]:.2f}', f'{mAP11_3d[j, k, i]:.2f}', f'{mAP11_aos[j, k, i]:.2f}',
                         f'{mAP40_bbox[j, k, i]:.2f}', f'{mAP40_bev[j, k, i]:.2f}', f'{mAP40_3d[j, k, i]:.2f}',
                         f'{mAP40_aos[j, k, i]:.2f}'])
                    ret_dict[f"{difficulty[k]}@{min_overlaps[i, 0, j]}@{curcls_name}@R11@APBEV"] = mAP11_bev[j, k, i]
                    ret_dict[f"{difficulty[k]}@{min_overlaps[i, 0, j]}@{curcls_name}@R11@AP3D"] = mAP11_3d[j, k, i]
                    ret_dict[f"{difficulty[k]}@{min_overlaps[i, 0, j]}@{curcls_name}@R11@AOS"] = mAP11_aos[j, k, i]
                    ret_dict[f"{difficulty[k]}@{min_overlaps[i, 0, j]}@{curcls_name}@R40@APBEV"] = mAP40_bev[j, k, i]
                    ret_dict[f"{difficulty[k]}@{min_overlaps[i, 0, j]}@{curcls_name}@R40@AP3D"] = mAP40_3d[j, k, i]
                    ret_dict[f"{difficulty[k]}@{min_overlaps[i, 0, j]}@{curcls_name}@R40@AOS"] = mAP40_aos[j, k, i]
                ret_dict[f"{difficulty[k]}@{min_overlaps[i, 0, j]}@{curcls_name}@R11@AP2D"] = mAP11_bbox[j, k, i]
                ret_dict[f"{difficulty[k]}@{min_overlaps[i, 0, j]}@{curcls_name}@R40@AP2D"] = mAP40_bbox[j, k, i]
    ...
    for i in range(min_overlaps.shape[0]):
        ...
        for k in range(len(difficulty)):
            if 'bev' not in eval_types:
                table_data.append(
                    [difficulty[k], f'{np.mean(min_overlaps[i, 0, :]):.2f}', curcls_name, f'{np.mean(mAP11_bbox[:, k, i]):.2f}',
                     f'{np.mean(mAP40_bbox[:, k, i]):.2f}'])
            else:
                table_data.append(
                    [difficulty[k], f'{np.mean(min_overlaps[i, 0, :]):.2f}', curcls_name, f'{np.mean(mAP11_bbox[:, k, i]):.2f}',
                     f'{np.mean(mAP11_bev[:, k, i]):.2f}', f'{np.mean(mAP11_3d[:, k, i]):.2f}',
                     f'{np.mean(mAP11_aos[:, k, i]):.2f}',
                     f'{np.mean(mAP40_bbox[:, k, i]):.2f}', f'{np.mean(mAP40_bev[:, k, i]):.2f}',
                     f'{np.mean(mAP40_3d[:, k, i]):.2f}',
                     f'{np.mean(mAP40_aos[:, k, i]):.2f}'])
                ret_dict[f"{difficulty[k]}@{np.mean(min_overlaps[i, 0, :]):.2f}@{curcls_name}@R11@APBEV"] = np.mean(
                    mAP11_bev[:, k, i])
                ret_dict[f"{difficulty[k]}@{np.mean(min_overlaps[i, 0, :]):.2f}@{curcls_name}@R11@AP3D"] = np.mean(
                    mAP11_3d[:, k, i])
                ret_dict[f"{difficulty[k]}@{np.mean(min_overlaps[i, 0, :]):.2f}@{curcls_name}@R11@AOS"] = np.mean(
                    mAP11_aos[:, k, i])
                ret_dict[f"{difficulty[k]}@{np.mean(min_overlaps[i, 0, :]):.2f}@{curcls_name}@R40@APBEV"] = np.mean(
                    mAP40_bev[:, k, i])
                ret_dict[f"{difficulty[k]}@{np.mean(min_overlaps[i, 0, :]):.2f}@{curcls_name}@R40@AP3D"] = np.mean(
                    mAP40_3d[:, k, i])
                ret_dict[f"{difficulty[k]}@{np.mean(min_overlaps[i, 0, :]):.2f}@{curcls_name}@R40@AOS"] = np.mean(
                    mAP40_aos[:, k, i])
            ret_dict[f"{difficulty[k]}@{np.mean(min_overlaps[i, 0, :]):.2f}@{curcls_name}@R11@AP2D"] = np.mean(
                mAP11_bbox[:, k, i])
            ret_dict[f"{difficulty[k]}@{np.mean(min_overlaps[i, 0, :]):.2f}@{curcls_name}@R40@AP2D"] = np.mean(
                mAP40_bbox[:, k, i])
