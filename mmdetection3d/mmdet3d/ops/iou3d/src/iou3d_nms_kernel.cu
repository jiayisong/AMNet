...
__device__ inline float box_overlap_bev(const float *box_a, const float *box_b){
    ...
    float a_dx_half = box_a[3] / 2, b_dx_half = box_b[3] / 2, a_dy_half = box_a[5] / 2, b_dy_half = box_b[5] / 2;
    float a_x1 = box_a[0] - a_dx_half, a_y1 = box_a[2] - a_dy_half;
    float a_x2 = box_a[0] + a_dx_half, a_y2 = box_a[2] + a_dy_half;
    float b_x1 = box_b[0] - b_dx_half, b_y1 = box_b[2] - b_dy_half;
    float b_x2 = box_b[0] + b_dx_half, b_y2 = box_b[2] + b_dy_half;

    Point center_a(box_a[0], box_a[2]);
    Point center_b(box_b[0], box_b[2]);
...
__device__ inline float box_overlap_3d(const float *box_a, const float *box_b){
    // params box_a: [x, y, z, dx, dy, dz, heading]
    // params box_b: [x, y, z, dx, dy, dz, heading]
    float top = fmaxf(box_a[1] - box_a[4] / 2, box_b[1] - box_b[4] / 2), bottom = fminf(box_a[1] + box_a[4] / 2, box_b[1] + box_b[4] / 2);
    float height = fmaxf(bottom - top, 0.f);
    return box_overlap_bev(box_a, box_b) * height;
}

__device__ inline float iou_bev(const float *box_a, const float *box_b){
    // params box_a: [x, y, z, dx, dy, dz, heading]
    // params box_b: [x, y, z, dx, dy, dz, heading]
    float sa = box_a[3] * box_a[5];
    float sb = box_b[3] * box_b[5];
    float s_overlap = box_overlap_bev(box_a, box_b);
...
__device__ inline float iou_3d(const float *box_a, const float *box_b){
    // params box_a: [x, y, z, dx, dy, dz, heading]
    // params box_b: [x, y, z, dx, dy, dz, heading]
    float sa = box_a[3] * box_a[4] * box_a[5];
    float sb = box_b[3] * box_b[4] * box_b[5];

    float s_overlap = box_overlap_3d(box_a, box_b);
    return s_overlap / fmaxf(sa + sb - s_overlap, EPS);
}

__global__ void boxes_overlap_bev_kernel(const int num_a, const float *boxes_a, const int num_b, const float *boxes_b, float *ans_overlap){
    ...
    float s_overlap = box_overlap_bev(cur_box_a, cur_box_b);
    ans_overlap[a_idx * num_b + b_idx] = s_overlap;
}

__global__ void boxes_overlap_3d_kernel(const int num_a, const float *boxes_a, const int num_b, const float *boxes_b, float *ans_overlap){
    // params boxes_a: (N, 7) [x, y, z, dx, dy, dz, heading]
    // params boxes_b: (M, 7) [x, y, z, dx, dy, dz, heading]
    const int a_idx = blockIdx.y * THREADS_PER_BLOCK + threadIdx.y;
    const int b_idx = blockIdx.x * THREADS_PER_BLOCK + threadIdx.x;
    ...
    const float * cur_box_a = boxes_a + a_idx * 7;
    const float * cur_box_b = boxes_b + b_idx * 7;
    float s_overlap = box_overlap_3d(cur_box_a, cur_box_b);
    ans_overlap[a_idx * num_b + b_idx] = s_overlap;
}
...
__global__ void boxes_iou_3d_aligned_kernel(const int num_a, const float *boxes_a, const float *boxes_b, float *ans_iou){
    // params boxes_a: (N, 7) [x, y, z, dx, dy, dz, heading]
    // params boxes_b: (M, 7) [x, y, z, dx, dy, dz, heading]
    ...
    const float * cur_box_a = boxes_a + a_idx * 7;
    const float * cur_box_b = boxes_b + a_idx * 7;
    float cur_iou_3d = iou_3d(cur_box_a, cur_box_b);
    ans_iou[a_idx] = cur_iou_3d;
}
...
__global__ void nms_bev_kernel(const int boxes_num, const float nms_overlap_thresh,
                           const float *boxes, unsigned long long *mask){
    //params: boxes (N, 7) [x, y, z, dx, dy, dz, heading]
    //params: mask (N, N/THREADS_PER_BLOCK_NMS)
...
void boxesoverlapbevLauncher(const int num_a, const float *boxes_a, const int num_b, const float *boxes_b, float *ans_overlap){
    ...
    boxes_overlap_bev_kernel<<<blocks, threads>>>(num_a, boxes_a, num_b, boxes_b, ans_overlap);
#ifdef DEBUG
...
void boxesoverlap3dLauncher(const int num_a, const float *boxes_a, const int num_b, const float *boxes_b, float *ans_overlap){
    ...
    boxes_overlap_3d_kernel<<<blocks, threads>>>(num_a, boxes_a, num_b, boxes_b, ans_overlap);
#ifdef DEBUG
    cudaDeviceSynchronize();  // for using printf in kernel function
#endif
}
...
void boxesiou3dalignedLauncher(const int num_a, const float *boxes_a, const float *boxes_b, float *ans_iou){
    ...
    dim3 blocks(DIVUP(num_a, THREADS_PER_BLOCK));
    dim3 threads(THREADS_PER_BLOCK);

    boxes_iou_3d_aligned_kernel<<<blocks, threads>>>(num_a, boxes_a,boxes_b, ans_iou);
#ifdef DEBUG
    cudaDeviceSynchronize();  // for using printf in kernel function
#endif
}
...
void nmsbevLauncher(const float *boxes, unsigned long long * mask, int boxes_num, float nms_overlap_thresh){
    dim3 blocks(DIVUP(boxes_num, THREADS_PER_BLOCK_NMS),
                DIVUP(boxes_num, THREADS_PER_BLOCK_NMS));
    dim3 threads(THREADS_PER_BLOCK_NMS);
    nms_bev_kernel<<<blocks, threads>>>(boxes_num, nms_overlap_thresh, boxes, mask);
}