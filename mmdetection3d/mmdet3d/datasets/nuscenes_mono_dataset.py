...
class NuScenesMonoDataset(CocoDataset):
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        print_log('Start to convert detection format...')
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            ...
            boxes, attrs = boxes_per_frame, attrs_per_frame
            ...
            cam_boxes3d_for_nms = cam_boxes3d.tensor[:, :7]
            ...
            cam_boxes3d = CameraInstance3DBoxes(boxes3d, box_dim=9, origin=(0.5, 0.5, 0.5))
        ...
        print_log('Results writes to', res_path)
    ...
    def evaluate(self,
        ...
        if isinstance(result_files, dict):
            ...
            for name in result_names:
                print_log('Evaluating bboxes of {}'.format(name), logger=logger)
                ret_dict = self._evaluate_single(result_files[name], logger=logger)
            results_dict.update(ret_dict)
        elif isinstance(result_files, str):
            results_dict = self._evaluate_single(result_files, logger=logger)
...
def output_to_nusc_box(detection):
    ...
    for i in range(len(box3d)):
        ...
        elif box3d.box_dim == 9:
            velocity = (box3d.tensor[i, 7], 0.0, box3d.tensor[i, 8])
        else:
            raise RuntimeError('box3d.box_dim error')
        box = NuScenesBox(
...
def nusc_box_to_cam_box3d(boxes):
    ...
    """
    locs = torch.from_numpy(np.array([b.center for b in boxes])).view(-1, 3)
    dims = torch.from_numpy(np.array([b.wlh for b in boxes])).view(-1, 3)
    rots = torch.from_numpy(np.array([b.orientation.yaw_pitch_roll[0] for b in boxes])).view(-1, 1)
    velocity = torch.from_numpy(np.array([b.velocity[0::2] for b in boxes])).view(-1, 2)
