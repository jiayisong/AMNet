...
class NuScenesFrontMonoDataset(NuScenesMonoDataset):
        ...
        if isinstance(result_files, dict):
            ...
            if metric == 'img_bbox2d':
                ap_result_str, ap_dict = kitti_eval(
                    gt_annos, result_files, self.CLASSES,self.gpu_ids[0], eval_types=['bbox'])
            else:
                ap_result_str, ap_dict = kitti_eval(gt_annos, result_files,
                                                    self.CLASSES,self.gpu_ids[0])
            print_log('\n' + ap_result_str, logger=logger)
    ...
    def show(self, results, out_dir, show=False, pipeline=None, score_thr=0.3):
